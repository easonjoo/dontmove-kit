...
func main() {
	...
	if serialAdapter != nil {
		...
		// 事件扇出（internal/modem/fanout.go）：API 控制面与 SIP 服务器是
		// 两个消费者，但 Go channel 每条消息只交给一个接收者——上游把
		// callModem.Events() 的同一个 channel 直接发给两方，RING/ended
		// 事件被二者抢来抢去：API 抢到则手机永远收不到 INVITE（来电看
		// 起来完全没反应），SIP 抢到则 Console 状态缺失。FanOut 给每个
		// 消费者各复制一份完整事件流。
		apiEvents := make(chan modem.ModemEvent, 32)
		sipEvents := make(chan modem.ModemEvent, 32)
		...
		if settings.SIP.Enabled {
			...
			// Linphone 锁屏来电推送（FlexiAPI，见 internal/sip/linphone_push.go）：
			// Key/From/URL 由 start_cellbridge.sh 从 ~/.cellbridge/linphone_push_*
			// 注入环境变量（CB_LINPHONE_PUSH_KEY/FROM/URL），Key 为空 = 功能关闭。
			sipServer.SetLinphonePush(
				os.Getenv("CB_LINPHONE_PUSH_KEY"),
				os.Getenv("CB_LINPHONE_PUSH_URL"),
				os.Getenv("CB_LINPHONE_FROM"),
			)
